...
> **公式（DATA 帧）：** $D = \text{Max\_Frame\_Size} - 2$（扣除 2 字节协议头）

### 3.3 帧通用结构

仅 **DATA** 和 **DATA\_END** 帧使用固定 2 字节协议头：

| Byte 0 | Byte 1 | Byte 2 ~ (N-1) |
|--------|--------|----------------|
| Command (命令字) | Sequence (包序号) | Payload (固件数据) |

其余帧类型（START、METADATA、VERIFY、REBOOT、ACK、NACK）无 Sequence 字段，各字段按 §4 中的定义排列。

- **Command**：标识帧类型（见 [§4 协议帧定义](#4-协议帧定义)）。
- **Sequence**：包序号（0~255），仅 DATA / DATA_END 帧使用。
...
| Remaining Data | Byte 4+ | R | 当前 Block 剩余数据（DLC − 4 字节，板端会以 `1024 − block_acc` 封顶，避免 CAN FD 离散 DLC 填充干扰） |

**关于 CAN FD DLC 填充：** CAN FD 帧的 DLC 是离散值（{8, 12, 16, 20, 24, 32, 48, 64}）。当 DATA_END 帧长不足一个离散 DLC 时，上位机会填充到下一个离散长度。板端在拷贝剩余数据时以 `min(rem_len, 1024 − block_accumulated_len)` 截断，尾部的零填充不影响 checksum（累加和中加 0 不改变结果）。详情见 `can_fd_dlc_padding_fix.md`。
...
| `0x0B` | `BOOT_STATUS_FRAME_SIZE` | 帧长度不支持 | 换用支持的帧长度重试 |
| `0x0C` | `BOOT_STATUS_FW_TOO_BIG` | 固件大小超分区 | 固件超过分区容量，修剪后再试 |
...
| 帧长度不支持 | NACK (FRAME_SIZE), 停留 IDLE | 换用支持的帧长度重试 |
| 固件大小超分区 | NACK (FW_TOO_BIG), 停留 IDLE | 确认固件适配当前分区 |
...
| 定时器周期 | 1 ms | `BOOT_TASK_PERIOD_MS` |
| CAN RX 队列深度 | 256 | `BOOT_MSG_FIFO_SIZE` |
