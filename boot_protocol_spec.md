...
载荷大小 D = Max_Frame_Size − 2。Sequence 每个 Block 内从 0 递增。错序帧静默丢弃，由 Block 超时兜底。
...
|--------|--------|--------|------------|
| `0x10` (ACK) | 应答的命令字 | `0x00` (成功) | `0x00` (填充) |

| Byte 0 | Byte 1 | Byte 2 | Byte 3 ~ 7 |
|--------|--------|--------|------------|
| `0x11` (NACK) | 被拒的命令字 | error_code | `0x00` (填充) |
...
Block Checksum 失败时仅重发当前 Block。板端 Block 级超时（200ms 帧间隔）自动触发重传请求。MCU 可请求最多 20 次重传，上位机最多重试 3 次。
...
| 参数 | 值 | 说明 |
|------|-----|------|
| 全局超时 | 6 秒 | 无 CAN 活动 → 复位到 IDLE |
| Block 帧间隔超时 | 200 ms | 帧间隔 > 200ms → 请求重传 |
| Block 最大重试 | 20 次 | MCU 侧，耗尽 → 复位到 IDLE |
...
| 全局超时 | 复位到 IDLE | 重新发起 START |
| 错序/残留帧 | 静默丢弃 | Block 超时兜底 |
...
| Block 帧间隔超时 | 200 ms | `BOOT_BLOCK_TIMEOUT_MS` |
| Block 最大重试 | 20 次 | `BOOT_BLOCK_MAX_RETRIES` |
