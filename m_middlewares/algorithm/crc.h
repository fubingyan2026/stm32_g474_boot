...
/**
  ...
  * @retval         计算完的CRC32
  */
extern uint32_t get_CRC32_check_sum(const uint8_t* pch_message, uint32_t dw_length, uint32_t init_crc);

/**
  * @brief          CRC32校验函数
  * @param[in]      pch_message: 数据（末尾4字节为CRC32值）
  * @param[in]      dw_length: 数据+校验的总长度
  * @retval         1=校验通过，0=校验失败
  */
extern uint32_t verify_CRC32_check_sum(const uint8_t* pch_message, uint32_t dw_length);
...
/**
  ...
  */
extern void append_CRC32_check_sum(uint8_t* pch_message, uint32_t dw_length);
